...
from contrib.operators.json_to_ndjson import JsonToNdjsonOperator
...
schema_path = os.path.join(configuration.get("core", "dags_folder"),
                            'schemas/bigquery/app_store/reviews.json')

with DAG(
    dag_id='fetch_reviews_app_store',
    ...
    for app in apps:
        ...
        scraped_path = 'reviews/{}/'.format(app['app_name']) + \
                    'reviews_{{ execution_date.format("%Y%m%d") }}.json'
        
        gcs_sensor = GoogleCloudStorageObjectSensor(
            task_id='sensor_reviews_{}'.format(app['app_name']),
            bucket=Variable.get('itunes_scraper_bucket'),
            object=scraped_path,
            google_cloud_conn_id='itunes_scraper_conn'
        )
        
        get_reviews = GoogleCloudStorageDownloadOperator(
            task_id='get_reviews_{}'.format(app['app_name']),
            bucket=Variable.get('itunes_scraper_bucket'),
            object=scraped_path,
        ...
        transform_ndjson = JsonToNdjsonOperator(
            task_id='{}_to_ndjson'.format(app['app_name']),
            json_path=output_path.path,
            output_path=output_path.path
        )

        gcs_path = 'reviews/app_store/{}/'.format(app['app_name']) + \
                    'reviews_{{ execution_date.format("%Y%m%d") }}.json'
        ...
        load_to_bq = GoogleCloudStorageToBigQueryOperator(
            ...
            destination_project_dataset_table='sapporo.{}_app_store_reviews'\
                                                .format(app['app_name']\
                                                        .replace('.','_')),
        ...
        start_task >> gcs_sensor >> get_reviews >> \
            transform_ndjson >> upload_to_gcs >> \
